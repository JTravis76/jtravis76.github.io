...
Array.prototype.groupBy = function (filter) {
    ...
    var groupedList = [];
    for (var c = 0; c < this.length; c++) {
        var keyValue = filterType == "string" ? this[c].filter : filter(this[c]);
...
function orderObjectsArray(array, filter, isDescending) {
    ...
    for (var c = 0; c < array.length; c++) {
        ...
        var firstValue = filterType === "string" ? array[c].filter : filter(array[c]);
        var secondValue = filterType === "string" ? array[c + 1].filter : filter(array[c + 1]);
